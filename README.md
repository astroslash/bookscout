...
`data/books/catalog.source.json` holds the original 200-book proposal, the focused coverage expansion, approved profiles, and separate submissions. AI-curator-approved classifications remain marked `reviewed: false`. `npm run catalog:build` validates the source and generates `data/books/catalog.json`; `npm run catalog:check` verifies that artifact. The build runs the check before compiling, so a stale generated catalog cannot deploy. The runtime imports the generated catalog into the Vercel server bundle. See [docs/CURATION.md](docs/CURATION.md) for commands and provenance and [docs/COVERAGE_CURATION.md](docs/COVERAGE_CURATION.md) for the educator and child-reader sources behind the focused expansion.
